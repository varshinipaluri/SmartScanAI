...
def start_http_server():
    ...
    server = subprocess.Popen(
        ["python", "-m", "http.server", "8000", "--directory", UPLOAD_DIR],
...
if not hasattr(st.session_state, 'http_server'):
    st.session_state.http_server = start_http_server()
...
def display_sources(sources, processor):
    """Display sources with clickable links and download buttons"""
    if not sources:
        return
    ...
    with st.expander("📚 Sources"):
        for source in sources:
            if source in processor.source_files:  # PDF file
                ...
                with col1:
                    # Clickable PDF link that opens in new tab
                    st.markdown(
                        f'<a href="{file_url}" class="pdf-source" target="_blank" '
                        f'style="text-decoration: none; margin-right: 10px;">'
                        f'📄 {file_name}</a>',
                        unsafe_allow_html=True
                    )
                with col2:
                    with open(file_info["path"], "rb") as f:
                        st.download_button(
                            ...
                            key=f"dl_{hash(file_info['path']) & 0xFFFFF}"
                        )
